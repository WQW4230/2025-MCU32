...
✨功能特性概览
🧠 双 MCU 协作（ESP32-S3 做视觉 + STM32 做系统控制）
📷 人脸检测 + 特征点识别 + 姿态解算（Yaw/Pitch/Roll）
🎮 多级 OLED 菜单系统 + 红外遥控 + 内置小游戏
🎛️ 自研轻量多任务系统 FunOS（仿 FreeRTOS 非抢占式调度）
🎚️ 三轴云台平滑控制（摇杆、红外、自动追踪、动作表）
💡 WS2812 灯珠 / LED 状态控制（巡逻 / 报警 / 闪光拍照）
💾 SD 卡存储 JPEG 1600×1200 高清抓拍
🔗 自定义 UART 通信协议
...
更新日志 2025/11/28
WatchBot_Link 2.6
STM32-LED控制菜单新增巡逻模式（红白蓝交替闪烁）、警报模式改为红蓝快速切换
ESP32在巡逻模式下led切换为巡逻模式，检测到人脸侵入切换回红蓝爆闪
接线图更新、物料清单完整更新。
...
更新日志 2025/12/16
WatchBot_Link 2.8
补丁：主频从160Mhz设置成240Mhz（这个我真的是太逆天了，做了几个月的项目才发现频率一直是160的低频
项目开始时候我会先配置好系统文件，硬件参数配置这些才开始写逻辑，模块，就不管这些硬件配置了。结果棋差一着，复盘时候发现了
有时候真的被自己无语到了）
更新工程主要模块以及核心模块的流程图和指令表


至此该项目功能已经全部实现，
作为我的第一个开源项目，虽有诸多不完美，却承载了本人几个月的心血
特别佩服、感谢那些有开源精神的大佬，碰到诸多问题，经常能从开源项目、帖子、视频中寻找到答案
虽然该项目价值低、功能实现简单。但依旧开源，希望能帮助到有需要的人
至此 --- 再见！
